...
//Lempel_ziv debe devolver un archivo binario con la compresion del mensaje y el trie para descomprimir el archivo
//Implementar trie
std::pair<trie, std::string> Lempel_Ziv(std::string mensaje){
    trie compression_trie = new trie();
    ...
    for(int i=0 ; i<largo ; i++){
        ...
        if(mensaje[i] == ' ' && !palabra.empty()){
            ...
            rango_palabra = compression_trie.find(palabra);
        ...
        rango_caracter = compression_trie.find(mensaje[i]);
        ...
        if(rango_caracter == nullptr){
            ...
            tmp_codificacion.append("("+mensaje[i]+')');
        } else{
            tmp_codificacion.append("("+mensaje[i]+','+std::to_string(rango_caracter->first)+')');
        }
    }
}
